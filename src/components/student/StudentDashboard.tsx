...
import {
  ...
  TryoutSubType,
  TRYOUT_SUB_TYPES,
  getAvailableTryoutSubTypes,
  isTryoutSubTypeAllowedForClass,
  resolveExamType
} from '../../types';
import {
  BarChart,
...
import { getStudentProgramInfo } from '../../utils/labschoolHelpers';
...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  // Exam List Type & Subtype Filter States
  const [examMainTypeFilter, setExamMainTypeFilter] = useState<'ALL' | ExamMainType>('ALL');
  const [tryoutSubTypeFilter, setTryoutSubTypeFilter] = useState<'ALL' | TryoutSubType>('ALL');
  const [examSearch, setExamSearch] = useState('');
  const [examCatFilter, setExamCatFilter] = useState('ALL');

  // Filter available Tryout Sub-Types specifically matched to this student's class
  // TO-SNBT -> XII-UTBK, TO-SMP LABSCHOOL -> SMP-LABSCHOOL, TO-SMA LABSCHOOL -> SMA-LABSCHOOL,
  // TO-TKA SD -> VI-SD, TO-TKA SMP -> IX-SMP, TO-TKA SMA -> XII-SMA
  const availableTryoutSubTypes = useMemo(() => {
    return getAvailableTryoutSubTypes(user.className);
  }, [user.className]);

  // Auto-reset tryoutSubTypeFilter if current selection is not available for this student's class
  useEffect(() => {
    if (tryoutSubTypeFilter !== 'ALL') {
      const isAllowed = availableTryoutSubTypes.some(sub => sub.id === tryoutSubTypeFilter);
      if (!isAllowed) {
        setTryoutSubTypeFilter('ALL');
      }
    }
  }, [availableTryoutSubTypes, tryoutSubTypeFilter]);
  ...
  // Computed count of exams by type
  const examCounts = useMemo(() => {
    let tryout = 0;
    let quiz = 0;
    let ulangan = 0;
    const tryoutSubCounts: Record<string, number> = {};

    studentExams.forEach(e => {
      const resolved = resolveExamType(e);
      if (resolved.mainType === 'TRYOUT') {
        tryout++;
        if (resolved.tryoutSubType) {
          tryoutSubCounts[resolved.tryoutSubType] = (tryoutSubCounts[resolved.tryoutSubType] || 0) + 1;
        }
      } else if (resolved.mainType === 'QUIZ') {
        quiz++;
      } else if (resolved.mainType === 'ULANGAN') {
        ulangan++;
      }
    });
  ...
  // Filtered exams according to active tabs, search, and category
  const filteredStudentExams = useMemo(() => {
    return studentExams.filter(exam => {
      const resolved = resolveExamType(exam);

      // Main type filter (ALL, TRYOUT, QUIZ, ULANGAN)
      if (examMainTypeFilter !== 'ALL' && resolved.mainType !== examMainTypeFilter) {
        return false;
      }

      // Tryout SubType filter (if main type is TRYOUT and a specific sub-type is selected)
      if (
        examMainTypeFilter === 'TRYOUT' &&
        tryoutSubTypeFilter !== 'ALL' &&
        resolved.tryoutSubType !== tryoutSubTypeFilter
      ) {
        return false;
      }

      // Category filter
      if (examCatFilter !== 'ALL' && exam.category !== examCatFilter) {
        return false;
      }
      ...
      if (examSearch.trim()) {
        ...
        const matchSub = (resolved.tryoutSubType || '').toLowerCase().includes(q);
        if (!matchTitle && !matchCat && !matchSub) return false;
      }

      return true;
    });
  }, [studentExams, examMainTypeFilter, tryoutSubTypeFilter, examCatFilter, examSearch]);
    ...
    <div className="space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              {(() => {
                const progInfo = getStudentProgramInfo(user);
                return (
                  <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-bold shadow-sm ${progInfo.badgeClass}`}>
                    <Award className="w-3.5 h-3.5" />
                    <span>{progInfo.label}</span>
                  </div>
                );
              })()}
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-950 border border-cyan-800/60 text-cyan-300 text-xs font-semibold">
                <Sparkles className="w-3.5 h-3.5 text-cyan-400" /> Kelas: {user.className || 'Belum Ditentukan'}
              </div>
              {user.group && (
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-950 border border-purple-800/60 text-purple-300 text-xs font-semibold">
                  <Layers className="w-3.5 h-3.5 text-purple-400" /> {user.group}
                </div>
              )}
              {(user.whatsapp || user.phone) && (
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-950 border border-emerald-800/60 text-emerald-300 text-xs font-semibold font-mono">
                  <MessageCircle className="w-3.5 h-3.5 text-emerald-400" /> {user.whatsapp || user.phone}
                </div>
              )}
              {user.nis && (
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div className="flex items-center gap-2.5">
              <div className="p-2.5 bg-cyan-950/80 border border-cyan-500/30 text-cyan-400 rounded-2xl shadow-inner">
                <FileCheck2 className="w-5 h-5" />
              </div>
              <div>
                <h3 className="font-extrabold text-lg text-white">
                  Daftar Ujian Aktif Kelas {user.className || 'Umum'}
                </h3>
                <p className="text-xs text-slate-400">
                  Pilih kelompok jenis ujian: Tryout Seleksi, Quiz Interaktif, atau Ulangan & Penilaian Berkala.
                </p>
              </div>
            </div>

            <div className="flex items-center gap-2">
              <span className="px-3 py-1 bg-slate-900 border border-slate-800 rounded-xl text-xs font-bold text-slate-300">
                Total: <strong className="text-cyan-400">{studentExams.length}</strong> Ujian Tersedia
              </span>
            </div>
          </div>

          {/* 1. PRIMARY EXAM TYPE TABS (Semua, Tryout, Quiz, Ulangan) */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-slate-900/90 border border-slate-800 p-2.5 rounded-2xl shadow-xl">
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none">
              <button
                type="button"
                onClick={() => {
                  setExamMainTypeFilter('ALL');
                  setTryoutSubTypeFilter('ALL');
                }}
                className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 shrink-0 cursor-pointer ${
                  examMainTypeFilter === 'ALL'
                    ? 'bg-gradient-to-r from-cyan-600 to-blue-600 text-white shadow-lg shadow-cyan-600/30'
                    : 'text-slate-400 hover:text-white hover:bg-slate-800'
                }`}
              >
                <span>Semua Ujian</span>
                <span className="px-1.5 py-0.2 rounded-md text-[10px] bg-slate-950/60 font-mono">
                  {examCounts.all}
                </span>
              </button>
              ...
              <button
                type="button"
                onClick={() => {
                  setExamMainTypeFilter('QUIZ');
                  setTryoutSubTypeFilter('ALL');
                }}
                className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 shrink-0 cursor-pointer ${
                  examMainTypeFilter === 'QUIZ'
              ...
              <button
                type="button"
                onClick={() => {
                  setExamMainTypeFilter('ULANGAN');
                  setTryoutSubTypeFilter('ALL');
                }}
                className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 shrink-0 cursor-pointer ${
                  examMainTypeFilter === 'ULANGAN'
            ...
            <div className="flex items-center gap-2 shrink-0">
              <div className="relative w-40 sm:w-48">
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 focus:border-cyan-500 rounded-xl py-1.5 pl-8 pr-3 text-xs text-white placeholder-slate-500 focus:outline-none"
                />
              </div>
              ...
              {categories.length > 0 && (
                <select
                  value={examCatFilter}
                  onChange={e => setExamCatFilter(e.target.value)}
                ...
                >
                  ...
                  {categories.map((c, idx) => (
                    <option key={`dash-cat-${c.id}-${idx}`} value={c.name}>
                      {c.name}
                    </option>
                  ))}
                </select>
          ...
          {/* 2. SUB-TYPE TRYOUT TABS (When TRYOUT is active - Filtered by Student Class Target) */}
          {examMainTypeFilter === 'TRYOUT' && (
            <div className="p-3.5 bg-indigo-950/40 border border-indigo-500/30 rounded-2xl space-y-2.5 animate-in fade-in duration-150 shadow-lg">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-xs">
                <span className="font-bold text-indigo-300 flex items-center gap-1.5">
                  <Layers className="w-3.5 h-3.5 text-cyan-400" /> Bagian / Sub-Kategori Tryout:
                </span>
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-slate-400">
                    Target Kelas: <strong className="text-cyan-300 font-semibold">{user.className || 'Umum'}</strong>
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  type="button"
                  onClick={() => setTryoutSubTypeFilter('ALL')}
                ...
                >
                  Semua Tryout ({examCounts.tryout})
                </button>

                {availableTryoutSubTypes.map(subType => {
                  const count = examCounts.tryoutSubCounts[subType.id] || 0;
                  const isActive = tryoutSubTypeFilter === subType.id;
                  return (
                    <button
                      key={subType.id}
                      type="button"
                      onClick={() => setTryoutSubTypeFilter(subType.id)}
                      className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer ${
                        isActive
                          ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-md border border-indigo-400/40'
                          : 'bg-slate-900 text-slate-300 hover:bg-slate-800 border border-slate-800'
                      }`}
                      title={`${subType.description} (${subType.targetGroup})`}
                    >
                      <span>{subType.label}</span>
                      <span className="px-1.5 py-0.2 rounded text-[9px] bg-slate-950/70 text-indigo-300 font-mono border border-indigo-500/20">
                        {subType.targetClass}
                      </span>
                      {count > 0 && (
                        <span className={`px-1.5 py-0.2 rounded text-[10px] font-mono font-bold ${
                          isActive ? 'bg-slate-950/80 text-cyan-300' : 'bg-slate-800 text-slate-400'
                        }`}>
                          {count}
                        </span>
                      )}
          ...
          {filteredStudentExams.length === 0 ? (
            <div className="p-12 text-center bg-slate-900 border border-slate-800 rounded-3xl space-y-3">
              ...
              <p className="text-sm font-bold text-slate-300">
                Tidak ada paket ujian untuk filter ini.
              </p>
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                Silakan pilih kategori/bagian lain atau hapus pencarian untuk menampilkan paket ujian yang tersedia.
              </p>
              {(examMainTypeFilter !== 'ALL' || tryoutSubTypeFilter !== 'ALL' || examSearch || examCatFilter !== 'ALL') && (
                <button
                  type="button"
                  onClick={() => {
                    setExamMainTypeFilter('ALL');
                    setTryoutSubTypeFilter('ALL');
                    setExamSearch('');
                    setExamCatFilter('ALL');
                  }}
                  className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-cyan-400 font-bold text-xs rounded-xl border border-slate-700 transition-colors"
                >
                  Reset Semua Filter
                </button>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredStudentExams.map(exam => {
                const resolvedType = resolveExamType(exam);
                ...
                return (
                  ...
                  >
                    <div className="space-y-3.5">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {/* Main Type Badge */}
                          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border ${resolvedType.bgClass} ${resolvedType.colorClass} ${resolvedType.borderClass}`}>
                            {resolvedType.badgeLabel}
                          </span>

                          {/* Tryout SubType Badge (if Tryout) */}
                          {resolvedType.mainType === 'TRYOUT' && resolvedType.tryoutSubType && (
                            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-blue-950/80 text-blue-300 border border-blue-800/60">
                              {resolvedType.tryoutSubType}
                            </span>
                          )}

                          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-cyan-950/80 text-cyan-300 border border-cyan-800/50">
                            {exam.category}
                          </span>
                          ...
                          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${
                            ...
                            {exam.mode === 'EMBED_DRIVE_PDF' ? 'PDF Split-View' : 'Native CBT'}
                          </span>

                          {exam.isIRTEnabled && (
                            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-950 text-indigo-300 border border-indigo-700">
                              IRT 3PL
                            </span>
                          )}
                          ...
                          {exam.subtests && exam.subtests.length > 0 && (
                            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-950 text-slate-300 border border-slate-700 flex items-center gap-1">
                              <Layers className="w-3 h-3 text-cyan-400" /> {exam.subtests.length} Subtes
                            </span>
                          )}
                        </div>
                    ...
                    <button
                      ...
                      className={`w-full py-3 text-white font-bold rounded-2xl text-xs transition-all flex items-center justify-center gap-2 cursor-pointer ${
                        isLimitReached
      ...
      {activeTab === 'overview' && (
        <div className="space-y-8">
          ...
          {/* SPLIT CALENDAR, AGENDA, AND PENGUMUMAN WIDGET FOR STUDENT */}
          <DashboardCalendarAgendaPengumuman
            user={user}
            onStartExam={(examId) => {
              const ex = exams.find(e => e.id === examId);
              if (ex) onStartExam(ex);
            }}
          />
      ...
      {/* TAB: LAPORAN TRYOUT & EVALUASI SISWA */}
                    ...
                    >
                      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        <div className="space-y-1">
                          ...
                          {res.subtestResults && res.subtestResults.length > 0 && (
                            <div className="flex items-center gap-1.5 pt-1 flex-wrap">
                              {res.subtestResults.map(st => (
                                <span
                                  key={st.subtestId}
                                  className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-slate-950 border border-slate-800 text-slate-300"
                                >
                                  <strong className="text-cyan-400">{st.subtestCode || st.subtestName}:</strong> {st.score} {res.isIRTScore ? 'IRT' : 'Poin'}
                                </span>
                              ))}
                            </div>
                          )}
          ...
          <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-50 flex items-center justify-center p-3 sm:p-5">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-7 max-w-4xl w-full max-h-[92vh] flex flex-col space-y-5 shadow-2xl relative overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto space-y-6 pr-1 custom-scrollbar">
                ...
                {/* IRT 3PL STATISTICAL SCORE BREAKDOWN */}
                {reviewResult.isIRTScore && (
                  <div className="p-5 bg-gradient-to-r from-indigo-950/70 to-slate-950 border border-indigo-800/60 rounded-3xl space-y-4 shadow-xl">
                    <div className="flex items-center justify-between border-b border-indigo-800/50 pb-2.5">
                      <div className="flex items-center gap-2">
                        <Calculator className="w-5 h-5 text-indigo-400" />
                        <h4 className="font-extrabold text-sm sm:text-base text-white">
                          Analisis Nilai Teori Respon Butir (Item Response Theory 3PL)
                        </h4>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div className="bg-slate-900/90 p-3.5 rounded-2xl border border-indigo-900/40">
                        <span className="text-[10px] text-slate-400 uppercase font-bold">Estimasi Kemampuan (Theta θ)</span>
                        <p className="text-xl font-black font-mono text-cyan-300 mt-1">
                          {reviewResult.irtTheta !== undefined ? (reviewResult.irtTheta >= 0 ? `+${reviewResult.irtTheta.toFixed(2)}` : reviewResult.irtTheta.toFixed(2)) : '0.00'}
                        </p>
                        <span className="text-[10px] text-slate-400 mt-1 block">
                          {(reviewResult.irtTheta || 0) >= 1.5 ? 'Tingkat Kemampuan Sangat Tinggi' :
                           (reviewResult.irtTheta || 0) >= 0.5 ? 'Tingkat Kemampuan Di Atas Rata-rata' :
                           (reviewResult.irtTheta || 0) >= -0.5 ? 'Tingkat Kemampuan Rata-rata' :
                           'Tingkat Kemampuan Perlu Bimbingan'}
                        </span>
                      </div>

                      <div className="bg-slate-900/90 p-3.5 rounded-2xl border border-indigo-900/40">
                        <span className="text-[10px] text-slate-400 uppercase font-bold">Skor Terstandar (Skala 200-1000)</span>
                        <p className="text-xl font-black font-mono text-amber-400 mt-1">
                          {reviewResult.irtStandardScore || reviewResult.score} <span className="text-xs font-normal text-slate-500">/ 1000</span>
                        </p>
                        <span className="text-[10px] text-slate-400 mt-1 block">Skor terbobot daya beda & kesulitan butir</span>
                      </div>

                      <div className="bg-slate-900/90 p-3.5 rounded-2xl border border-indigo-900/40">
                        <span className="text-[10px] text-slate-400 uppercase font-bold">Peringkat Persentil (Percentile)</span>
                        <p className="text-xl font-black font-mono text-emerald-400 mt-1">
                          {reviewResult.irtPercentile || 50}%
                        </p>
                        <span className="text-[10px] text-slate-400 mt-1 block">
                          Mengungguli {reviewResult.irtPercentile || 50}% peserta se-Indonesia
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {/* MULTI-SUBTEST PERFORMANCE BREAKDOWN */}
                {reviewResult.subtestResults && reviewResult.subtestResults.length > 0 && (
                  <div className="p-5 bg-slate-950/80 border border-slate-800 rounded-3xl space-y-4 shadow-xl">
                    <div className="flex items-center justify-between border-b border-slate-800/80 pb-2.5">
                      <div className="flex items-center gap-2">
                        <Layers className="w-5 h-5 text-cyan-400" />
                        <h4 className="font-extrabold text-sm sm:text-base text-white">
                          Rincian Nilai Per-Kelompok Subtes ({reviewResult.subtestResults.length} Subtes)
                        </h4>
                      </div>
                      <span className="text-xs text-slate-400 font-medium">Dipisahkan per materi subtes</span>
                    </div>
                      ...
                      <table className="w-full text-left text-xs text-slate-300">
                        <thead className="bg-slate-950 text-slate-400 uppercase tracking-wider text-[10px] font-semibold border-b border-slate-800">
                          <tr>
                            ...
                            <th className="p-3">Nama Subtes</th>
                            <th className="p-3 text-center">Soal</th>
                            <th className="p-3 text-center">B / S / K</th>
                            <th className="p-3 text-center">Skor Subtes</th>
                            <th className="p-3 text-center">Akurasi</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-800/60">
                          {reviewResult.subtestResults.map((st, idx) => {
                            const pct = st.percentage || (st.totalQuestions > 0 ? Math.round((st.correctCount / st.totalQuestions) * 100) : 0);
                            return (
                              <tr key={st.subtestId || idx} className="hover:bg-slate-800/40">
                                <td className="p-3 font-mono font-bold text-cyan-400">
                                  {st.subtestCode || `ST${idx + 1}`}
                                </td>
                                ...
                                <td className="p-3 text-center font-mono">
                                  <span className="text-emerald-400 font-bold">{st.correctCount}</span> /{' '}
                                  <span className="text-rose-400 font-bold">{st.incorrectCount}</span> /{' '}
                                  <span className="text-slate-400">{st.unansweredCount}</span>
                                </td>
                                <td className="p-3 text-center">
                                  ...
                                  {reviewResult.isIRTScore && (
                                    <span className="text-[9px] text-slate-500 block">/ 1000 IRT</span>
                                  )}
                                </td>
                                <td className="p-3 text-center">
                                  <div className="flex items-center justify-center gap-2">
                                    <div className="w-14 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                                      <div className="h-full bg-cyan-400 rounded-full" style={{ width: `${Math.min(100, pct)}%` }} />
                                    </div>
                    ...
                    {/* Subtests Comparative Chart */}
                    <div className="h-44 w-full pt-1">
                      <p className="text-[11px] font-bold text-slate-400 mb-2">Grafik Perbandingan Nilai Antar Subtes:</p>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                          data={reviewResult.subtestResults.map(st => ({
                            name: st.subtestCode || st.subtestName,
                            skor: st.score,
                            persentase: st.percentage || 0,
                            fullName: st.subtestName
                          }))}
                        ...
                        >
                              ...
                              if (active && payload && payload.length) {
                                ...
                                return (
                                  <div className="bg-slate-900 border border-slate-700 p-2.5 rounded-xl text-xs space-y-1 shadow-xl">
                                    <p className="font-extrabold text-cyan-300">{d.fullName}</p>
                                    <p className="text-white font-bold">Skor: {d.skor} Poin</p>
                                    <p className="text-slate-300">Akurasi: {d.persentase}%</p>
                                  </div>
                                );
                              }
                              return null;
                          ...
                          />
                          <Bar dataKey="skor" fill="#06b6d4" radius={[8, 8, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
