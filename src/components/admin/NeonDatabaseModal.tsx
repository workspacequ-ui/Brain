...
import { Database, CheckCircle2, XCircle, RefreshCw, UploadCloud, DownloadCloud, Sparkles, X, Key, ShieldCheck, HelpCircle, Layers } from 'lucide-react';
import { checkNeonStatus, bulkLoadFromNeon, NeonStatus } from '../../utils/neonClient';
import {
...
export const NeonDatabaseModal: React.FC<NeonDatabaseModalProps> = ({
  ...
  const handleSyncToNeon = async () => {
    ...
    if (result.neonCloudSync.synced) {
      onShowToast?.(result.message, 'success');
      fetchStatus();
    } else {
      onShowToast?.(result.neonCloudSync.message || result.message, 'info');
    }
  };
  ...
  const handleLoadFromNeon = async () => {
    ...
    if (!remoteData || Object.keys(remoteData).length === 0) {
      onShowToast?.('Tidak ada data tersimpan di Neon DB atau koneksi belum terkonfigurasi', 'error');
      return;
    }

    let restoredCount = 0;
    if (remoteData[KEYS.USERS] || remoteData.bsa_users) {
      saveUsers(remoteData[KEYS.USERS] || remoteData.bsa_users);
      restoredCount++;
    }
    if (remoteData[KEYS.TEACHERS] || remoteData.bsa_teachers) {
      saveTeachers(remoteData[KEYS.TEACHERS] || remoteData.bsa_teachers);
      restoredCount++;
    }
    if (remoteData[KEYS.CLASSES] || remoteData.bsa_classes) {
      saveClasses(remoteData[KEYS.CLASSES] || remoteData.bsa_classes);
      restoredCount++;
    }
    if (remoteData[KEYS.SUBJECTS] || remoteData.bsa_subjects) {
      saveSubjects(remoteData[KEYS.SUBJECTS] || remoteData.bsa_subjects);
      restoredCount++;
    }
    if (remoteData[KEYS.CATEGORIES] || remoteData.bsa_categories) {
      saveCategories(remoteData[KEYS.CATEGORIES] || remoteData.bsa_categories);
      restoredCount++;
    }
    if (remoteData[KEYS.MATERIALS] || remoteData.bsa_materials) {
      saveMaterials(remoteData[KEYS.MATERIALS] || remoteData.bsa_materials);
      restoredCount++;
    }
    if (remoteData[KEYS.EXAMS] || remoteData.bsa_exams) {
      saveExams(remoteData[KEYS.EXAMS] || remoteData.bsa_exams);
      restoredCount++;
    }
    if (remoteData[KEYS.RESULTS] || remoteData.bsa_results) {
      saveResults(remoteData[KEYS.RESULTS] || remoteData.bsa_results);
      restoredCount++;
    }
    if (remoteData[KEYS.PRODUCTS] || remoteData.bsa_products) {
      saveProducts(remoteData[KEYS.PRODUCTS] || remoteData.bsa_products);
      restoredCount++;
    }
    if (remoteData[KEYS.MARKETPLACE_CATEGORIES] || remoteData.bsa_mkt_categories) {
      saveMarketplaceCategories(remoteData[KEYS.MARKETPLACE_CATEGORIES] || remoteData.bsa_mkt_categories);
      restoredCount++;
    }
    if (remoteData[KEYS.FEATURED_PROGRAMS] || remoteData.bsa_featured_programs) {
      saveFeaturedPrograms(remoteData[KEYS.FEATURED_PROGRAMS] || remoteData.bsa_featured_programs);
      restoredCount++;
    }
    if (remoteData[KEYS.SYLLABUS] || remoteData.bsa_syllabus) {
      saveSyllabi(remoteData[KEYS.SYLLABUS] || remoteData.bsa_syllabus);
      restoredCount++;
    }
    if (remoteData[KEYS.AGENDAS] || remoteData.bsa_agendas) {
      saveAgendas(remoteData[KEYS.AGENDAS] || remoteData.bsa_agendas);
      restoredCount++;
    }
    if (remoteData[KEYS.ANNOUNCEMENTS] || remoteData.bsa_announcements) {
      saveAnnouncements(remoteData[KEYS.ANNOUNCEMENTS] || remoteData.bsa_announcements);
      restoredCount++;
    }
    if (remoteData[KEYS.SETTINGS] || remoteData.bsa_app_settings) {
      saveAppSettings(remoteData[KEYS.SETTINGS] || remoteData.bsa_app_settings);
      restoredCount++;
    }
    if (remoteData[KEYS.INSTITUTION] || remoteData.bsa_institution_info) {
      saveInstitutionInfo(remoteData[KEYS.INSTITUTION] || remoteData.bsa_institution_info);
      restoredCount++;
    }

    onShowToast?.(`Berhasil memulihkan ${restoredCount} modul data dari database Neon PostgreSQL!`, 'success');
    setTimeout(() => {
