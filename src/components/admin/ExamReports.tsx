          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-400">
            ...
            >
              ...
              {categories.map((cat, idx) => (
                <option key={`rep-cat-${cat.id}-${idx}`} value={cat.name}>{cat.name}</option>
              ))}
            </select>
          </div>
                    ...
                    <tr key={res.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="p-4 font-medium text-slate-300 print:text-black">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {res.isIRTScore && (
                            <span className="px-1.5 py-0.2 rounded text-[9px] font-extrabold bg-indigo-950 text-indigo-300 border border-indigo-700">
                              IRT 3PL
                            </span>
                          )}
                        </div>
                        ...
                        {res.subtestResults && res.subtestResults.length > 0 && (
                          <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                            {res.subtestResults.map(st => (
                              <span
                                key={st.subtestId}
                                className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-slate-950 border border-slate-800 text-slate-400"
                                title={`${st.subtestName}: ${st.score} poin (${st.correctCount}/${st.totalQuestions} Benar)`}
                              >
                                <strong className="text-cyan-300">{st.subtestCode || st.subtestName.substring(0, 4)}:</strong> {st.score}
                              </span>
                            ))}
                          </div>
                        )}
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          {res.isIRTScore ? (
                            <span className="text-[9px] font-mono text-indigo-400 font-bold">
                              / 1000 IRT {res.irtTheta !== undefined ? `(θ:${res.irtTheta >= 0 ? '+' : ''}${res.irtTheta.toFixed(1)})` : ''}
                            </span>
                          ) : (
      ...
      {selectedDetailResult && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-3xl w-full space-y-6 shadow-2xl my-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3.5 bg-slate-950 border border-slate-800 rounded-2xl">
                <span className="text-[10px] text-slate-400 font-bold uppercase">
                  {selectedDetailResult.isIRTScore ? 'Skor IRT 3PL' : 'Skor Akhir'}
                </span>
                <p className="text-2xl font-black text-amber-400 mt-1">
                  {selectedDetailResult.score}{' '}
                  <span className="text-xs font-normal text-slate-500">
                    / {selectedDetailResult.maxScore || (selectedDetailResult.isIRTScore ? 1000 : 100)}
                  </span>
                </p>
                {selectedDetailResult.isIRTScore && selectedDetailResult.irtTheta !== undefined && (
                  <span className="text-[10px] text-indigo-400 font-mono font-bold block mt-0.5">
                    θ: {selectedDetailResult.irtTheta >= 0 ? '+' : ''}{selectedDetailResult.irtTheta.toFixed(2)} (Persentil {selectedDetailResult.irtPercentile || 50}%)
                  </span>
                )}
              </div>
            ...
            {/* IRT 3PL Statistical Analytics Card */}
            {selectedDetailResult.isIRTScore && (
              <div className="p-4 bg-gradient-to-r from-indigo-950/60 to-slate-950 border border-indigo-800/60 rounded-2xl space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Calculator className="w-4 h-4 text-indigo-400" />
                    <h4 className="font-extrabold text-indigo-200 text-sm">
                      Analisis Teori Respon Butir (Item Response Theory 3PL)
                    </h4>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="bg-slate-900/90 p-3 rounded-xl border border-indigo-900/40">
                    ...
                    <p className="text-lg font-black font-mono text-cyan-300 mt-0.5">
                      {selectedDetailResult.irtTheta !== undefined ? (selectedDetailResult.irtTheta >= 0 ? `+${selectedDetailResult.irtTheta.toFixed(2)}` : selectedDetailResult.irtTheta.toFixed(2)) : '0.00'}
                    </p>
                    <span className="text-[10px] text-slate-400">
                      {(selectedDetailResult.irtTheta || 0) >= 1.5 ? 'Tingkat Kemampuan Sangat Tinggi' :
                       (selectedDetailResult.irtTheta || 0) >= 0.5 ? 'Tingkat Kemampuan Di Atas Rata-rata' :
                       (selectedDetailResult.irtTheta || 0) >= -0.5 ? 'Tingkat Kemampuan Rata-rata' :
                  ...
                  <div className="bg-slate-900/90 p-3 rounded-xl border border-indigo-900/40">
                    ...
                    <p className="text-lg font-black font-mono text-amber-400 mt-0.5">
                      {selectedDetailResult.irtStandardScore || selectedDetailResult.score}
                    </p>
                    <span className="text-[10px] text-slate-400">Skor terbobot daya beda & kesulitan butir</span>
                  </div>
                  ...
                  <div className="bg-slate-900/90 p-3 rounded-xl border border-indigo-900/40">
                    ...
                    <p className="text-lg font-black font-mono text-emerald-400 mt-0.5">
                      {selectedDetailResult.irtPercentile || 50}%
                    </p>
                    <span className="text-[10px] text-slate-400">
                      Mengungguli {selectedDetailResult.irtPercentile || 50}% peserta lainnya
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* Multi-Subtest Granular Breakdown Section */}
            {selectedDetailResult.subtestResults && selectedDetailResult.subtestResults.length > 0 && (
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  <div className="flex items-center gap-2">
                    <Layers className="w-5 h-5 text-indigo-400" />
                    <h4 className="font-extrabold text-white text-base">
                      Laporan & Analisis Nilai Per-Kelompok Subtes ({selectedDetailResult.subtestResults.length} Subtes)
                    </h4>
                  </div>
                  <span className="text-[11px] text-slate-400">Dipisahkan per kelompok materi</span>
                </div>

                {/* Subtest Table */}
                <div className="bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden">
                  <table className="w-full text-left text-xs text-slate-300">
                    <thead className="bg-slate-900 text-slate-400 uppercase tracking-wider text-[10px] font-semibold border-b border-slate-800">
                      <tr>
                        <th className="p-3">Kode / Subtes</th>
                        <th className="p-3">Nama Kelompok Subtes</th>
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {selectedDetailResult.subtestResults.map((st, idx) => {
                        const pct = st.percentage || (st.totalQuestions > 0 ? Math.round((st.correctCount / st.totalQuestions) * 100) : 0);
                        const statusColor =
                          pct >= 80 ? 'bg-emerald-950 text-emerald-300 border-emerald-800' :
                          pct >= 60 ? 'bg-blue-950 text-blue-300 border-blue-800' :
                          pct >= 40 ? 'bg-amber-950 text-amber-300 border-amber-800' :
                          'bg-rose-950 text-rose-300 border-rose-800';
                        const statusLabel =
                          pct >= 80 ? 'Sangat Baik' :
                          pct >= 60 ? 'Baik' :
                          pct >= 40 ? 'Cukup' : 'Perlu Bimbingan';

                        return (
                          <tr key={st.subtestId || idx} className="hover:bg-slate-900/40">
                ...
                {/* Subtest Comparison Bar Chart */}
                <div className="p-4 bg-slate-950 border border-slate-800 rounded-2xl space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-bold text-indigo-300 flex items-center gap-1.5">
                      <BarChart3 className="w-4 h-4 text-indigo-400" /> Perbandingan Nilai Antar Subtes
                    </p>
                    <span className="text-[10px] text-slate-400">Skala Skor Subtes</span>
                  </div>

                  <div className="h-44 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={selectedDetailResult.subtestResults.map(st => ({
                      ...
                      >
                            ...
                            if (active && payload && payload.length) {
                              ...
                              return (
                                <div className="bg-slate-900 border border-slate-700 p-2.5 rounded-xl text-xs space-y-1 shadow-xl">
                                  <p className="font-extrabold text-indigo-300">{d.fullName}</p>
                                  <p className="text-white font-bold">Skor: {d.skor} Poin</p>
                                  <p className="text-cyan-400">Pencapaian: {d.persentase}%</p>
                                </div>
                              );
                            }
                            return null;
                        ...
                        />
                        <Bar dataKey="skor" fill="#6366f1" radius={[8, 8, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
            ...
            <form onSubmit={handleSaveSubmit} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  >
                    {categories.map((cat, idx) => (
                      <option key={`rep-modal-cat-${cat.id}-${idx}`} value={cat.name}>{cat.name}</option>
                    ))}
                  </select>
                </div>
              </div>
